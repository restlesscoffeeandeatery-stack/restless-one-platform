...
function platformRpcDoPost_(payload){
  ...
  var handlers={
    ...
    getAppData:getAppData,getAppDataJson:getAppDataJson,saveTransaction:saveTransaction,updateTransaction:updateTransaction,saveCategory:saveCategory,saveSupplier:saveSupplier,
  ...
  try{
    var args=Array.isArray(payload.args)?payload.args:[],cache=CacheService.getScriptCache(),cacheable=['getPlatformData','getInventoryWorkspace','getPayrollAdminData','getAppData','getAppDataJson','getDashboard','getBahanBaku','getProduk','getPreparations'],cacheKey='AIO_RPC_'+name;
...
function postPayroll(key,runId,accountId){platformAuth_(key);var ss=platformSS_(),sh=ss.getSheetByName(PLATFORM.PAY_RUN),row=platformFindRow_(sh,runId);if(!row)throw new Error('Payroll tidak ditemukan');var r=sh.getRange(row,1,1,10).getValues()[0];if(r[4]==='POSTED')return{ok:true,already:true,transactionId:String(r[7]||'')};if(!accountId)throw new Error('Pilih rekening pembayaran');var finance=platformFinanceSS_(),financeTx=finance.getSheetByName('Transaksi');platformAssertFinanceSchema_(financeTx);var marker='PAYROLL_RUN:'+runId,existing=platformFindTransactionByReference_(financeTx,marker),txId=existing||'TX-'+Utilities.getUuid(),now=new Date(),startDate=platformDate_(r[2]),endDate=platformDate_(r[3]),description='Payroll '+r[1]+' '+startDate+' s/d '+endDate,values=[txId,platformParseIsoDate_(endDate),'Pengeluaran',platformNum_(r[5]),description,'GAJI',r[1]==='Fulltime'?'GAJI BULANAN':'GAJI MINGGUAN PARTTIME','Transfer',marker,String(accountId),'',marker,endDate.slice(0,7),now,now];var lock=LockService.getScriptLock();lock.waitLock(20000);try{if(!existing)financeTx.appendRow(values);if(!platformFindTransactionByReference_(ss.getSheetByName(PLATFORM.FIN_TX),marker))ss.getSheetByName(PLATFORM.FIN_TX).appendRow(values);sh.getRange(row,5,1,6).setValues([['POSTED',r[5],accountId,txId,r[8],now]]);platformAudit_(ss,'PAYROLL','POST_KEUANGAN_BARU',runId,txId);return{ok:true,transactionId:txId,writtenTo:finance.getName()};}finally{lock.releaseLock();}}

/** Jalankan sekali untuk memperbaiki tanggal/deskripsi transaksi payroll lama. Aman diulang. */
function migrasiPerbaikiTanggalTransaksiPayroll(){
  var props=PropertiesService.getScriptProperties();platformAuth_(props.getProperty('PLATFORM_ADMIN_KEY')||props.getProperty('PLATFORM_ADMIN_PIN'));
  var ss=platformSS_(),runSheet=ss.getSheetByName(PLATFORM.PAY_RUN),runMap={},updated=0,checked=0,seenSheets={};
  platformRows_(runSheet,10).forEach(function(run){runMap[String(run[0])]=run;});
  ['Transaksi',PLATFORM.FIN_TX].forEach(function(sheetName){
    var sheet=ss.getSheetByName(sheetName);if(!sheet||seenSheets[sheet.getSheetId()])return;seenSheets[sheet.getSheetId()]=true;
    var rows=platformRows_(sheet,15);
    rows.forEach(function(transaction,index){
      var reference=String(transaction[11]||'');if(reference.indexOf('PAYROLL_RUN:')!==0)return;checked++;
      var runId=reference.slice('PAYROLL_RUN:'.length),run=runMap[runId];if(!run)return;
      var startDate=platformDate_(run[2]),endDate=platformDate_(run[3]),description='Payroll '+String(run[1])+' '+startDate+' s/d '+endDate,period=endDate.slice(0,7);
      if(platformDate_(transaction[1])===endDate&&String(transaction[4])===description&&String(transaction[12])===period)return;
      transaction[1]=platformParseIsoDate_(endDate);transaction[4]=description;transaction[12]=period;transaction[14]=new Date();
      sheet.getRange(index+2,1,1,15).setValues([transaction]);updated++;
    });
  });
  platformAudit_(ss,'PAYROLL','MIGRASI_TANGGAL_TRANSAKSI','PAYROLL_RUN','Diperiksa '+checked+'; diperbaiki '+updated);
  var result={ok:true,checked:checked,updated:updated,message:updated?'Tanggal transaksi payroll berhasil diperbaiki.':'Semua tanggal transaksi payroll sudah benar.'};Logger.log(JSON.stringify(result));return result;
}
...
function platformGetPayrollAdmin_(ss){var employeeSheet=ss.getSheetByName('Karyawan'),attendanceSheet=ss.getSheetByName('Absensi'),employees=platformRows_(employeeSheet,6).map(function(r){return{id:String(r[0]),name:String(r[1]),type:String(r[2]),rate:platformNum_(r[3]),status:String(r[4]),startDate:platformDate_(r[5])};}),lastRow=attendanceSheet?attendanceSheet.getLastRow():0,count=Math.min(400,Math.max(0,lastRow-1)),attendanceRows=count?attendanceSheet.getRange(lastRow-count+1,1,count,11).getValues():[],attendance=attendanceRows.reverse().map(function(r){return{id:String(r[0]),employeeId:String(r[1]),date:platformDate_(r[2]),inTime:platformCellTime_(r[3]),outTime:platformCellTime_(r[4]),hours:platformNum_(r[5]),status:String(r[6]),note:String(r[7]||''),overtimeHours:platformNum_(r[9]),overtimeRate:platformNum_(r[10])};});return{employees:employees,attendance:attendance};}
