...
function savePayrollEmployee(key,data){platformAuth_(key);data=data||{};var ss=platformSS_(),sh=ss.getSheetByName(PLATFORM.PAY_EMP),id=String(data.id||'')||'EMP-'+Utilities.getUuid(),row=platformFindRow_(sh,id),name=String(data.name||'').trim(),type=String(data.type||'Fulltime'),rate=platformNum_(data.rate);if(!name||['Fulltime','Parttime'].indexOf(type)<0||rate<=0)throw new Error('Nama, tipe, dan rate karyawan wajib valid');var values=[id,name,type,rate,String(data.status||'Aktif'),String(data.attendanceName||name),new Date()];if(row)sh.getRange(row,1,1,7).setValues([values]);else sh.appendRow(values);platformAudit_(ss,'PAYROLL',row?'EDIT_KARYAWAN':'TAMBAH_KARYAWAN',id,name);return{ok:true,id:id};}
function deletePayrollEmployee(key,id){platformAuth_(key);var ss=platformSS_(),sh=ss.getSheetByName(PLATFORM.PAY_EMP),row=platformFindRow_(sh,id);if(!row)throw new Error('Karyawan tidak ditemukan');sh.getRange(row,5).setValue('Nonaktif');platformAudit_(ss,'PAYROLL','NONAKTIF_KARYAWAN',id,'');return{ok:true};}
function savePayrollAttendance(key,data){platformAuth_(key);data=data||{};var ss=platformSS_(),sh=ss.getSheetByName(PLATFORM.PAY_ATT),id=String(data.id||'')||'ATT-'+Utilities.getUuid(),row=platformFindRow_(sh,id),employeeId=String(data.employeeId||''),date=platformDate_(data.date),status=String(data.status||'Hadir');if(!employeeId||!date||['Hadir','Izin','Sakit','Alpha'].indexOf(status)<0)throw new Error('Karyawan, tanggal, dan status absensi wajib valid');var values=[id,employeeId,date,String(data.inTime||''),String(data.outTime||''),platformNum_(data.hours),status,String(data.note||''),platformNum_(data.overtimeHours),platformNum_(data.overtimeRate),new Date()];if(row)sh.getRange(row,1,1,11).setValues([values]);else sh.appendRow(values);platformAudit_(ss,'PAYROLL',row?'EDIT_ABSENSI':'TAMBAH_ABSENSI',id,status);return{ok:true,id:id};}
function deletePayrollAttendance(key,id){platformAuth_(key);var ss=platformSS_(),sh=ss.getSheetByName(PLATFORM.PAY_ATT),row=platformFindRow_(sh,id);if(!row)throw new Error('Absensi tidak ditemukan');sh.deleteRow(row);platformAudit_(ss,'PAYROLL','HAPUS_ABSENSI',id,'');return{ok:true};}
...
function refreshPayrollLive(key,runId){platformAuth_(key);var ss=platformSS_(),sync=platformSyncPayrollRealtime_(ss),run=null;if(runId)run=sync.changed?platformRefreshPayrollRun_(ss,String(runId)):platformReadPayrollRun_(ss,String(runId));return platformSafe_({sync:sync,payrollAdmin:platformGetPayrollAdmin_(ss),run:run,checkedAt:platformDateTime_(new Date())});}
...
function platformGetPayrollAdmin_(ss){var legacyStart={};platformRows_(ss.getSheetByName('Karyawan'),6).forEach(function(r){legacyStart[String(r[0])]=platformDate_(r[5])});var employees=platformRows_(ss.getSheetByName(PLATFORM.PAY_EMP),7).map(function(r){return{id:String(r[0]),name:String(r[1]),type:String(r[2]),rate:platformNum_(r[3]),status:String(r[4]),attendanceName:String(r[5]||r[1]),startDate:legacyStart[String(r[0])]||''};}),attendance=platformRows_(ss.getSheetByName(PLATFORM.PAY_ATT),11).slice(-400).reverse().map(function(r){return{id:String(r[0]),employeeId:String(r[1]),date:platformDate_(r[2]),inTime:platformCellTime_(r[3]),outTime:platformCellTime_(r[4]),hours:platformNum_(r[5]),status:String(r[6]),note:String(r[7]||''),overtimeHours:platformNum_(r[8]),overtimeRate:platformNum_(r[9])};});return{employees:employees,attendance:attendance};}
...
function platformCalculatePayroll_(ss,scheme,period,runId,manualDeltas){
  var empSheet=ss.getSheetByName(PLATFORM.PAY_EMP),attSheet=ss.getSheetByName(PLATFORM.PAY_ATT);
  if(!empSheet||!attSheet)throw new Error('Sheet Payroll_Karyawan/Payroll_Absensi tidak ditemukan di spreadsheet master');
  var employees=platformRows_(empSheet,7).filter(function(r){return String(r[2])===scheme&&String(r[4]).toLowerCase()==='aktif';}),byEmployee={};
