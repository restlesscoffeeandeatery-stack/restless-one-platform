...
const Layout = () => {
  ...
  const currentUser = useStore(s => s.currentUser) || { name: 'Rayhan', role: 'Owner', initials: 'RR' };
    ...
    <div className="main-content">
      <header className="top-header">
        ...
        <div className="header-tools">
          ...
          <button className="profile-menu" type="button" onClick={logout} title={`Log out ${currentUser.name}`}><span className="user-avatar">{currentUser.initials}</span><span className="profile-copy"><strong>{currentUser.name}</strong><small>{currentUser.role}</small></span><LogOut size={16}/></button>
        </div>
      </header>
